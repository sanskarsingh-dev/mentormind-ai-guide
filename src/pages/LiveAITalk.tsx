...
const LiveAITalk = () => {
  ...
  const { mentorSlug } = useParams<{ mentorSlug: string }>(); // Added: Read mentor slug from URL
  ...
  // Added: Function to slugify mentor name for URL
  const slugify = (name: string) => name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');

  // Added: Effect to load mentor from URL param on mount
  useEffect(() => {
    if (mentorSlug) {
      const foundMentor = mentors.find((m) => slugify(m.name) === mentorSlug);
      if (foundMentor) {
        setSelectedMentor(foundMentor);
        setSelectedSubject(foundMentor.subject);
        setConnectionState("connected");
        setStatusText(`You are connected to ${foundMentor.name}`);
      }
    }
  }, [mentorSlug]);
  ...
  const handleSubjectSelect = (subject: string) => {
    ...
    if (mentor) {
      ...
      // Added: Navigate to URL with slugified mentor name
